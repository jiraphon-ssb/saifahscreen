...
import { Check, ShoppingBag, Plus, Trash2, Info, ChevronRight, Calculator } from "lucide-react";
...
export default function ProductPanel({ 
  ...
  const handleAddCurrentToOrder = () => {
    addOrder(config.tshirt.name, config.tshirt.colorValue, config.tshirt.imageUrl);
  }
        ...
        <div className="p-5 space-y-8 pb-20">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <h2 className="text-xl font-bold text-primary">1. เลือกประเภทเสื้อ</h2>
              <p className="text-xs text-zinc-400 font-medium tracking-normal">Premium หรือ Oversize ทรงที่เป็นคุณ</p>
            </div>
            ...
            >
                ...
                >
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-[10px] text-zinc-400 mt-1 font-medium">คอกลม/พอดีตัว</p>
                    </div>
                </Label>
                <Label 
                  htmlFor="oversize" 
                ...
                >
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-[10px] text-zinc-400 mt-1 font-medium">ทรงหลวมสไตล์สตรีท</p>
                    </div>
                </Label>
            </RadioGroup>
          </div>
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-zinc-950">2. เลือกสีเสื้อ</h2>
                <Badge variant="secondary" className="text-[10px] px-3 font-bold bg-primary/10 text-primary uppercase">CURRENT: {config.tshirt.name}</Badge>
            </div>
                    ...
                    >
                        <Image
                            ...
                            className={cn(
                                ...
                                config.tshirt.colorValue === tshirt.colorValue && "opacity-90"
                            )}
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-black/40 backdrop-blur-[2px] p-2 text-center">
                            <span className="text-white text-[11px] font-bold uppercase">{tshirt.label}</span>
                        </div>
                    </button>
                ))}
            </div>
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-primary">3. ระบุไซส์และจำนวน</h2>
            ...
            >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                      <p className="text-sm font-bold uppercase leading-none">เพิ่มรายการผลิตใหม่</p>
                      <p className="text-[10px] text-zinc-500 font-medium mt-1">ระบุไซส์สำหรับสีเสื้อที่เลือกอยู่ ({config.tshirt.name})</p>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              {orders.map((order) => (
                <div key={order.id} className="overflow-hidden bg-white border border-zinc-100 rounded-[24px] shadow-sm ring-1 ring-zinc-50">
                  <div className="px-4 py-3 bg-zinc-50/80 flex items-center justify-between border-b border-zinc-100">
                    <div className="flex items-center gap-3">
                        <div className="h-9 w-9 rounded-xl bg-white p-1 border border-zinc-200">
                            <img src={order.tshirt.imageUrl} alt={order.tshirt.name} className="h-full w-full object-contain" />
                        </div>
                        <p className="text-xs font-bold uppercase text-zinc-900 tracking-tight">{order.tshirt.name}</p>
                    </div>
                    <Button 
                        variant="ghost" size="icon" className="h-9 w-9 text-zinc-300 hover:text-destructive hover:bg-destructive/5 rounded-xl transition-all"
                  ...
                  <div className="p-5">
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div key={size} className="space-y-2">
                            <Label className="text-[10px] font-bold text-center block text-zinc-400">{size}</Label>
                            <Input 
                                type="number"
                                min="0"
                                value={order.sizes[size] || 0}
                                onChange={(e) => updateOrderSize(order.id, size, parseInt(e.target.value) || 0)}
            ...
            {orders.length === 0 && (
                <div className="p-10 text-center bg-zinc-50/50 rounded-[32px] border-2 border-dashed border-zinc-200 border-spacing-4">
                    ...
                    <p className="text-xs font-bold text-zinc-400 uppercase">ยังไม่มีรายการสั่งผลิตในตะกร้า</p>
                    <p className="text-[10px] text-zinc-300 mt-1 font-medium">กดปุ่มสีดำเพื่อเริ่มระบุจำนวนที่ต้องการ</p>
                </div>
            )}
          </div>
        </div>
      ...
      <div className="p-5 pb-8 border-t bg-white shadow-[0_-15px_30px_rgba(0,0,0,0.03)] z-20">
         <div className="flex items-center justify-between">
            <div className="flex gap-3 items-center">
                ...
                <div>
                    <p className="text-xs font-bold uppercase text-zinc-950 opacity-40">ยอดผลิตรวม</p>
                    <p className="text-[11px] font-bold text-primary">พร้อมสำหรับขั้นตอนการเช็คเอาท์</p>
                </div>
            </div>
            <div className="text-right">
                <span className="text-4xl font-bold font-prompt text-zinc-950 tracking-tighter">{totalQuantity}</span>
                <span className="text-xs font-bold text-zinc-300 ml-2 uppercase">ตัว (T-Shirt)</span>
            </div>
         </div>
      </div>
    </div>
