...
const defaultTshirt = PlaceHolderImages.find(
  (p) => p.id === "t-shirt-mockup-white-saifah",
);
...
const initialDesignState: DesignState = {
  productConfig: {
    ...
    tshirt: {
      name: "White",
      imageUrl:
        defaultTshirt?.imageUrl || "/images/t-shirt-mockup-white-saifah.png",
      colorValue: "#FFFFFF",
    },
  },
  elements: [],
...
export default function DesignTool({ designId }: DesignToolProps) {
  ...
  const updateOrderSize = (orderId: string, size: string, quantity: number) => {
    setDesignState(
      ...
      true,
    ); // Don't push to history for every keystroke if needed, but here simple is better
  };
  ...
  const selectedElement =
    designState.elements.find((el) => el.id === selectedElementId) || null;
      ...
      <div className="h-[100dvh] w-[100dvw] overflow-hidden bg-background text-foreground flex flex-col">
        ...
        >
          ...
          <div className="flex items-center gap-1 md:gap-3">
            ...
            <Button
              size="sm"
              className="gap-2 rounded-full font-semibold shadow-md hover:shadow-lg transition-all"
              onClick={handleShare}
            >
