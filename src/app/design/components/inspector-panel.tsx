...
import {
  Image as ImageIcon, Trash2, Type, AlignCenter, AlignLeft, AlignRight, FlipHorizontal, FlipVertical, Copy, ChevronDown, Move, Sliders
} from "lucide-react";
...
const LabeledControl = ({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) => (
    <div className={cn("grid gap-2", className)}>
        <Label className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">{label}</Label>
        {children}
    </div>
);
  ...
  return (
    <div className="flex flex-col h-full bg-white font-prompt">
        ...
        <div className="p-5 border-b border-zinc-100 bg-zinc-50/50">
            <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-primary flex items-center justify-center shadow-md shadow-primary/20">
                    ...
                    {isImage && <ImageIcon className="h-5 w-5 text-white stroke-[2.5]" />}
                </div>
                <div className="flex-1 min-w-0">
                    <Input 
                        value={selectedElement.name}
                        onChange={(e) => updateElement(selectedElement.id, { name: e.target.value }, false)}
                        onBlur={(e) => updateElement(selectedElement.id, { name: e.target.value }, true)}
                        className="font-bold text-base text-zinc-950 bg-transparent border-0 focus-visible:ring-0 p-0 h-auto tracking-tight"
                    />
                    <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{isText ? 'ข้อความ' : 'รูปภาพ'}</span>
                </div>
            </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <Accordion type="multiple" defaultValue={['transform', 'style']} className="w-full">
                ...
                <AccordionItem value="style" className="border-b-0">
                    ...
                    <AccordionContent className="px-5 pb-5 space-y-5 pt-1">
                        ...
                        <LabeledControl label={`ความโปร่งใส ${Math.round((selectedElement.opacity ?? 1) * 100)}%`}>
                            <Slider 
                            ...
                            <>
                                ...
                                <LabeledControl label="ฟอนต์">
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <Button variant="outline" className="w-full justify-between h-10 text-sm font-bold border-zinc-200 bg-zinc-50 rounded-xl px-3">
                                        ...
                                        <DropdownMenuContent className="w-64 max-h-[400px] overflow-y-auto bg-white border-zinc-200 rounded-2xl shadow-2xl p-2" align="start">
                                            <DropdownMenuLabel className="text-[10px] font-bold text-zinc-400 uppercase p-3">ฟอนต์ไทย</DropdownMenuLabel>
                                            {thaiFonts.map(font => (
                                                <DropdownMenuItem key={font.value} onSelect={() => updateElement(selectedElement.id, { fontFamily: font.value })} style={{fontFamily: font.value}} className="text-lg py-3 rounded-xl">
                                                    {font.name}
                                                </DropdownMenuItem>
                                            ))}
                                            <DropdownMenuSeparatorPrimitive className="bg-zinc-100 my-2" />
                                            <DropdownMenuLabel className="text-[10px] font-bold text-zinc-400 uppercase p-3">ฟอนต์อังกฤษ</DropdownMenuLabel>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <LabeledControl label="น้ำหนัก">
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                                <Button variant="outline" className="w-full justify-between h-9 text-sm font-bold border-zinc-200 bg-zinc-50 rounded-xl">
                                            ...
                                            <DropdownMenuContent className="bg-primary text-primary-foreground border-none rounded-xl">
                                                {fontWeights.map(w => (
                                                    <DropdownMenuItem key={w.value} onSelect={() => updateElement(selectedElement.id, { fontWeight: w.value })} className="font-bold focus:bg-white/10 focus:text-white">
                                                        {w.name}
                                                    </DropdownMenuItem>
                                                ))}
                                            </DropdownMenuContent>
                                        ...
                                        {[
                                            ...
                                            <Button
                                                ...
                                                className={cn(
                                                    ...
                                                    selectedElement.textAlign === align.value ? "bg-white text-zinc-950 shadow-sm" : "text-zinc-400 hover:text-zinc-600"
                                                )}
                                                onClick={() => updateElement(selectedElement.id, { textAlign: align.value as any })}
                                            >
                                ...
                                <div className="flex items-center justify-between p-3 bg-zinc-50 rounded-xl border border-zinc-100">
                                    <div className="space-y-0.5">
                                        <Label className="text-xs font-bold text-zinc-950">เงาตัวอักษร</Label>
                                    </div>
                        ...
                        {isImage && (
                            <>
                                <Separator className="bg-zinc-100" />
                                <LabeledControl label={`ความสว่าง ${selectedElement.filters?.brightness ?? 100}`}>
                                    <Slider 
                                ...
                                <LabeledControl label={`ความเข้ม ${selectedElement.filters?.contrast ?? 100}`}>
                                    <Slider 
        ...
        <div className="p-4 border-t border-zinc-100 bg-zinc-50/50 grid grid-cols-2 gap-3">
            <Button variant="secondary" className="h-10 font-bold rounded-xl bg-white border border-zinc-200 shadow-sm gap-2 text-sm" onClick={() => duplicateElement(selectedElement.id)}>
                <Copy className="h-4 w-4 text-zinc-600" /> ทำซ้ำ
            </Button>
            <Button variant="destructive" className="h-10 font-bold rounded-xl bg-red-600 hover:bg-red-700 shadow-sm gap-2 text-sm" onClick={() => deleteElement(selectedElement.id)}>
