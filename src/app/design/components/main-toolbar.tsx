...
const tools: ToolItem[] = [
  ...
  { id: 'Elements', icon: PlusSquare, label: 'เพิ่มองค์ประกอบ', shortLabel: 'เพิ่ม' },
  { id: 'Inspector', icon: SlidersHorizontal, label: 'แก้ไขคุณสมบัติ', shortLabel: 'แก้ไข', needsSelection: true },
                        ...
                        <Tooltip key={tool.id}>
                            <TooltipTrigger asChild>
                                <button
                                  ...
                                  disabled={isDisabled}
                                >
                                  <tool.icon className={cn("h-5 w-5 transition-transform", isActive && 'scale-110')} />
                                  <span className={cn(
                            ...
                            <TooltipContent side="right" className="bg-primary text-primary-foreground border-none shadow-2xl">
                                <p className="font-bold text-xs">{tool.label}</p>
                                {tool.needsSelection && <p className="text-[10px] opacity-70">(เลือกวัตถุก่อน)</p>}
                            </TooltipContent>
                        </Tooltip>
                    );
                })}
